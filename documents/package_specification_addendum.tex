...
\begin{center}
  ...
  {\large Model-matrix, initialization, and E-step implementation addendum}
\end{center}
...
are zero and their log values are negative infinity. The main outcome
log-likelihood function has not yet been implemented.

\section{Automated tests}

The model-matrix, initialization, and E-step infrastructure is covered by
automated tests implemented with the \texttt{tinytest} package. Separate test
files cover model matrices, misclassification expansion, cross-component
compatibility, initialization, and expectation calculations.

The tests verify capture-grid and design-matrix dimensions, row alignment,
factor-level order, all-zero capture cells, and capture-cell indices. They also
cover multiple misclassified variables, multiplication of confusion-matrix
contributions, and \texttt{true\_if} restrictions. Models without an outcome
or misclassification mechanism and both new compatibility errors are included.
The matrix, compatibility, and initialization tests exercise the implemented
behavior through \texttt{crc\_fit()}. The E-step is tested directly as an
internal function because it has not yet been integrated into the fitting
loop. All tests pass against the built and installed package.

Initialization tests additionally verify state-weight dimensions, finiteness,
non-negativity, within-observation normalization, capture-cell aggregation,
the default and custom concentration values, reproducibility under
\texttt{set.seed()}, the single-class case, and invalid-control handling.

The E-step tests verify the posterior-weight formula, normalization within
observed units, exact zero weights for impossible states, capture-cell
aggregation, all-zero-cell expectations, omission of the outcome contribution,
log-scale numerical stability, returned dimensions and class, and the main
validation failures. The current suite contains 56 passing assertions, of
which 14 exercise \texttt{expectation\_crc()}.

\section{Required next steps}

The model-matrix, initialization, and distribution-independent E-step
infrastructure and their automated tests are complete at the design level.
Development should continue in the following order.

\begin{enumerate}
  \item Implement state-aligned outcome log-likelihood contributions for exact,
  interval-censored, right-censored, and missing outcomes under the
  zero-truncated Poisson and negative binomial distributions. Use
  \texttt{log\_sub\_crc()} for stable interval probabilities.
