...
bindings. This separation allows the internal EM driver to reuse the same
preparation pipeline while unit tests exercise preparation without invoking
the complete fitting routine. The EM driver is not yet called by
\texttt{crc\_fit()}.
...
available as internal fitting components and are coordinated by the internal
EM algorithm, although that algorithm has not yet been integrated into
\texttt{crc\_fit()}.
...
iteration, but the resulting fitting loop is not yet called by
\texttt{crc\_fit()}.
...
the final update with \texttt{converged = FALSE}; the public response to this
condition will be defined when the loop is integrated into
\texttt{crc\_fit()}.
...
The model-matrix, initialization, control, E-step, both M-steps,
outcome-likelihood, and EM components are covered by automated tests
...
outcome log-likelihood calculations, and EM orchestration.
...
construct parsed inputs, model matrices, or initialization objects. The
E-step and both M-steps remain tested directly, while the dedicated EM tests
exercise their orchestration without invoking the still-unfitted public
\texttt{crc\_fit()} interface.
...
iteration limit. The complete suite now contains 128 passing assertions.

\section{Required next steps}

The model-matrix, initialization, control, distribution-independent E-step,
both M-steps, outcome-likelihood infrastructure, internal EM algorithm, and
their automated tests are complete at the design level. Development should
continue in the following order.

\begin{enumerate}
  \item Call \texttt{perform\_em\_crc()} from \texttt{crc\_fit()} and define
  the fitted public result structure while retaining the parsed specifications
  and prepared model components needed for interpretation and prediction.

  \item Define the public behavior for an exhausted EM iteration limit and
  implement user-facing progress messages for \texttt{verbose = TRUE}.

  \item Add end-to-end tests for the public fitting path, including multiple
  latent classes, misclassification, both outcome distributions, and models
  without an outcome, before exposing fitted estimates through methods.
\end{enumerate}
