...
\begin{center}
  ...
  public fitting and prediction interfaces}
\end{center}
...
\section{Deterministic simulation replication}
...
simulation implementation. Because it is a single deterministic replication,
it is not by itself evidence about finite-sample bias, variability, or
coverage.
...
\begin{enumerate}
  \item Conduct a broader simulation study over many independent replications.
  It should summarize overall and occupation-level bias and root mean squared
  error, convergence and failure rates, numerical boundary behavior, and
  computation time, with comparisons against the earlier IAOS implementation
  where appropriate.
...
implementation. The deterministic IAOS replication now establishes the initial
end-to-end integration check. The next development step is the broader
simulation study needed to evaluate finite-sample performance and computational
reliability across repeated samples. The existing automated tests establish the
fitting, prediction, matrix, alignment, iteration, and convergence invariants
