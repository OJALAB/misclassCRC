...
\begin{center}
  ...
  capture-model M-step, outcome likelihood, and outcome-model M-step addendum}
\end{center}
...
\texttt{prepare\_crc()}. It then transfers the prepared components into the
existing unfitted result without changing the public interface or result
structure. The local variables created through \texttt{list2env()} are
declared explicitly so that static R code analysis can identify their
bindings. This separation allows the future EM driver to reuse the same
preparation pipeline while unit tests exercise preparation without invoking
the complete fitting routine.
...
available as internal fitting components, although the complete EM algorithm
has not yet been integrated into \texttt{crc\_fit()}.
...
\texttt{crc\_fit()}, whose result remains in class
\texttt{crcfit\_unfitted} and has \texttt{fitted = FALSE}.
...
call \texttt{prepare\_crc()} directly instead of using \texttt{crc\_fit()} to
construct parsed inputs, model matrices, or initialization objects. The E-step
and both M-steps remain tested directly because they are not yet integrated
into the fitting loop. This separation prevents their unit tests from running
the future complete EM algorithm.
...
suite contains 105 passing assertions, of which nine exercise
\texttt{outcome\_loglik\_crc()}. The preparation refactor retains the same
datasets, random-number seeds, numerical expectations, error patterns, and
assertion count. Twelve fixture calls across seven test files now use
\texttt{prepare\_crc()}, and the model-matrix tests verify the
\texttt{crcfit\_preparation} class.
