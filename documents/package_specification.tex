...
\begin{center}
  ...
  and outcome-likelihood implementation addendum}
\end{center}
...
of these components is integrated into \texttt{crc\_fit()}. The capture-model
M-step is available as an internal fitting component, although the complete EM
algorithm has not yet been integrated into \texttt{crc\_fit()}.
...
It is not yet called by \texttt{crc\_fit()}, because the outcome M-step and
the complete EM iteration have not been implemented.
...
\section{Planned outcome-model maximization}

The outcome component of the M-step will maximize the posterior-weighted
...
The implementation will minimize $-Q_V$ with \texttt{optim()} using
\texttt{method = "BFGS"}. For \texttt{"ztpois"}, the parameter vector will
contain only the outcome regression coefficients $\beta$, with
$\mu_s=\exp(z_s^\mathsf{T}\beta)$. For \texttt{"ztnegbin"}, it will also
contain \texttt{log\_sigma}; optimizing the logarithm guarantees
$\sigma>0$ without box constraints. No arbitrary coefficient bounds or
user-selected optimizer will be exposed initially. Numerical derivatives will
be used in the first implementation; analytical derivatives may be added if
profiling demonstrates a material need.

For the first M-step, preliminary regression coefficients will be obtained
from a posterior-weighted Poisson fit to exact outcomes. If there are too few
exact outcomes for that fit, the intercept will be initialized as the logarithm
of the weighted mean of informative lower bounds, and the remaining
coefficients will be set to zero. For the negative binomial model, a positive
method-of-moments dispersion estimate based on exact outcomes will be used
when available, with 0.25 as the conservative fallback. These quantities are
starting values only and do not replace maximization of the zero-truncated,
censored objective.

After the first iteration, the preceding outcome coefficients and
\texttt{log\_sigma}, when applicable, will be supplied as warm starts. The
\texttt{max\_iter} and \texttt{relative\_tolerance} elements of
\texttt{control\$outcome} will be translated to the corresponding
\texttt{optim()} controls.

An update will be accepted only when \texttt{optim()} reports convergence code
zero, all parameters and the objective are finite, and the relevant
$Q_V$ value has not decreased beyond a small numerical tolerance. A failed
optimization will be retried once from the conservative first-iteration
values. If the retry also fails, fitting will stop with an informative error
rather than silently retaining stale parameters.

\section{Automated tests}

The model-matrix, initialization, control, E-step, capture-model M-step, and
...
control parsing, expectation calculations, capture-model maximization, and
...
suite now contains 91 passing assertions, of which nine exercise
...
capture-model M-step, and outcome-likelihood infrastructure and their
automated tests are complete at the design level. Development should continue
in the following order.

\begin{enumerate}
  \item Implement and test the outcome-model M-step according to the planned
  optimization and warm-start strategy above.
